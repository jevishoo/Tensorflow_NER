...
def set_test(test_iter, model_file):
    ...
    所需预测概率保存
    """
    if 'test' in test_iter.data_file:
...
if __name__ == '__main__':
    ...
    test_iter = DataIterator(config.train_batch_size, data_file=result_data_dir + 'test.txt', use_bert=config.use_bert,
    ...
    # test_iter = DataIterator(config.batch_size, data_file=result_data_dir + 'dev_test.txt', use_bert=config.use_bert,
    #                          seq_length=config.sequence_length, is_test=True, tokenizer=tokenizer)
    # set_test(test_iter, config.checkpoint_path)
